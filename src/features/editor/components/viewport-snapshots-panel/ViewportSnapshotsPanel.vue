...
<template>
  <aside
    ...
    class="flex h-full w-full min-w-0 flex-col overflow-hidden bg-bg-surface text-text-primary pointer-events-auto select-none"
    role="region"
  ...
  >
    ...
    <div class="shrink-0 border-b border-border-default bg-bg-surface p-3">
    ...
    <div data-tour="snapshots-list" class="custom-scrollbar min-h-0 flex-1 overflow-y-auto p-2.5">
      <div v-if="snapshotStore.snapshots.length > 0" class="space-y-2">
        <article
          ...
          :key="snapshot.id"
          class="snapshot-row group flex min-w-0 gap-2.5 rounded-xl border border-border-default bg-bg-elevated p-2 transition-colors duration-200 hover:border-primary/50 hover:bg-bg-surface-hover"
        >
          <div
            class="snapshot-thumbnail relative aspect-video w-28 shrink-0 overflow-hidden rounded-lg border border-border-default bg-bg-base"
          >
            <img
      ...
      <EmptyState
        ...
        class="min-h-44 border border-dashed border-border-default bg-bg-elevated px-3"
      />
    </div>

    <footer
      class="shrink-0 border-t border-border-default bg-bg-elevated px-3 py-2 text-[9px] leading-snug text-text-muted"
    >
