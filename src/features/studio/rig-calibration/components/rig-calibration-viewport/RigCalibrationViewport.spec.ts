...
import { SelectionTransformBox } from '@/components/ui/selection-transform-box'
...
describe('RigCalibrationViewport (Multi-Pièces)', () => {
  it('se monte correctement avec les props par défaut', () => {
    ...
    expect(wrapper.text()).not.toContain('Calibration du Rig')
    expect(wrapper.text()).not.toContain('800 × 900 px')
  })

  it('applique les coordonnées relatives de la tête sans HUD redondant', () => {
    const wrapper = mount(RigCalibrationViewport, {
      props: defaultProps
    })

    const transformBox = wrapper.getComponent(SelectionTransformBox)
    expect(transformBox.props('x')).toBe(400)
    expect(transformBox.props('y')).toBe(250)
  })
